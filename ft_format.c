...
/*   Updated: 2025/02/14 10:35:25 by dcaetano         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_format(const char *format, va_list args, int *ret)
{
	...
	if (*format == 'u')
		return (ft_putnbr_base(va_arg(args, unsigned int), 10, ret, NULL));
	if (*format == 'x')
		return (ft_putnbr_base(va_arg(args, unsigned int), 16, ret,
				ft_tolower));
	if (*format == 'X')
		return (ft_putnbr_base(va_arg(args, unsigned int), 16, ret,
				ft_toupper));
